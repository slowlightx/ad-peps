...
from adpeps.ipeps.ipeps_kitaev import iPEPS, iPEPS_exci
        ...
        for i in A.x_major():
            with cur_loc(i):
                if not nrms1.is_changed(0, 0):
                    ...
                    op_2 = np.exp(1j*(sim_config.px+sim_config.py)/3) * ops_exci[i_op+num_sites][0, 0]
                    op_exci += np.einsum('ia,jb,kc->ijkabc', idp2, op_2, idp2)
                    op_3 = np.exp(1j*(sim_config.px+sim_config.py)/3) * ops_exci[i_op+num_sites*2][0, 0]
                ...
                for i in A.x_major():
                    with cur_loc(i):
                        if not nrms1.is_changed(0, 0):
                            ...
                            op_2 = np.exp(1j * (sim_config.px + sim_config.py) / 3) * ops_exci[obs_i + num_sites][0, 0]
                            op_exci += np.einsum('ia,jb,kc->ijkabc', idp, op_2, idp)
                            op_3 = np.exp(1j * (sim_config.px + sim_config.py) / 3) * ops_exci[obs_i + num_sites * 2][0, 0]
